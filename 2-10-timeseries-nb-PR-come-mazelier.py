...
# %%
# à vous
df = pd.read_csv('data/Amazon.csv', header = 3)
df.dtypes
...
# %%
df2 = pd.DataFrame(df.loc['2018-07' : '2019-12']['High'].copy())
df2.plot()
df2.count()
#Il y a 377 valeurs affichées.
...
# %%
# à vous
df3 = pd.DataFrame(df2.resample('W').mean())
df3.count()
df3.plot()

# %%
df3.count()
#Il y a maintenant 79 valeurs données, ce qui est cohérent avec un resample
#à l'échelle de la semaine pour des données qui s'étalent sur un an et demi.
...
# %%
# à vous
df4 = pd.DataFrame(df['High'].rolling( window = pd.Timedelta(365,'D') ).mean())
df4.plot()
